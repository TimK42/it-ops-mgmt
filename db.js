...
function seedData() {
  ...
  const cats = [
    ['CAD', '#4f46e5', '📐'], ['GIS', '#16a34a', '🗺️'],
    ['PVNS', '#f97316', '📡'], ['ACOS', '#dc2626', '🛡️'],
  ];
  const catMap = {};
  ...
  const qas = [
    ['QA-0001', '如何重置使用者密碼', '使用者忘記密碼該如何處理？', 'Step 1: 確認身分。Step 2: 至 Admin → User Management → Reset Password。Step 3: 產生一次性連結寄送。', 'CAD', 'password,account', 'Published'],
    ['QA-0002', 'API 504 處理流程', 'Gateway timeout 時如何回應客戶？', '1. 確認是否單一用戶問題。2. curl 測試。3. 全面性問題通報 on-call。4. 單一問題建議清除 DNS cache。', 'GIS', 'api,timeout', 'Published'],
    ['QA-0003', 'VPN 連線設定', '如何連線公司內部 VPN？', 'Windows: 設定 → VPN → 新增 → server.company.com。Mac: 系統偏好 → 網路 → VPN。使用公司憑證登入。', 'PVNS', 'vpn,remote', 'Published'],
    ['QA-0004', '登入異常基本檢查', '使用者反應無法登入時的先檢查項目', '1. 帳號未鎖定？2. 密碼未過期？3. 瀏覽器版本？4. 清除 cache。5. 試無痕模式。', 'ACOS', 'login', 'Published'],
    ['QA-0005', '報表匯出格式說明', '系統支援哪些報表匯出格式？', '支援 CSV、Excel (.xlsx)、PDF。CSV: 大量資料。Excel: 含格式。PDF: 正式報告。', 'CAD', 'report,export', 'Draft'],
  ];
  for (const q of qas) {
