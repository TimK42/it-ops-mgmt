...
async function renderQA(el) {
  ...
  if (isFirstRender) {
    ...
    toolbar.innerHTML = `<h2 class="sr-only">Filters</h2><div class="qa-toolbar-inner"><div class="qa-toolbar-row qa-toolbar-row--filters"><div class="filter-group">${statuses.map((s) => `<button class="filter-tab ${state.qaFilters.status === s ? 'active' : ''}" data-qf="${s || ''}">${s || 'All'}</button>`).join('')}</div></div><div class="qa-toolbar-row qa-toolbar-row--controls"><div class="filter-group"><div class="search-box"><span class="search-icon">🔍</span><label for="global-search" class="sr-only">Search QA entries</label><input type="search" placeholder="Search..." id="global-search" inputmode="search"><button class="search-clear" id="search-clear" style="display:none" aria-label="Clear search">×</button></div><select id="qa-sort" class="sort-select" aria-label="Sort order"><option value="popular"${state.qaSort === 'popular' ? ' selected' : ''}>Popular</option><option value="newest"${state.qaSort === 'newest' ? ' selected' : ''}>Newest</option></select>${canEdit ? '<button class="btn btn-ghost btn-sm" data-action="export-csv">📥 Export</button>' : ''}${canEdit ? `<button class="btn btn-primary btn-sm" data-action="create-qa">＋ New Entry</button>` : ''}</div></div></div>`;
