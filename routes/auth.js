...
router.get('/me', (req, res) => {
  ...
  const db = getDb();
  const u = db.prepare('SELECT id, username, role, status, created_at FROM users WHERE id = ?').get(req.session.userId);
  if (!u || u.status !== 'active') {
    req.session.destroy(() => {});
    return res.status(401).json({ error: 'Account disabled' });
  }
  res.json({ id: u.id, username: u.username, role: u.role, status: u.status, created_at: u.created_at });
});
...
router.post('/register', (req, res) => {
  ...
  const r = role || 'Viewer';
  if (!['Admin','Contributor','Viewer'].includes(r)) return res.status(400).json({ error: 'Invalid role' });
