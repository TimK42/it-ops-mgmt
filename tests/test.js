...
function req(method, urlPath, opts = {}) {
  return new Promise((resolve) => {
    ...
    r.on("error", () =>
      resolve({ status: -1, body: "", json: null, setCookie: [], headers: {} }),
    );
...
async function run() {
  ...
  const server = spawn(
    ...
    [
      "-e",
      "require('./server').listen(3199,'127.0.0.1',()=>console.log('ready'))",
    ],
    {
  ...
  try {
    ...
    assert(
      js.includes('aria-label="Search QA entries"'),
      "JS: search aria-label",
    );
    assert(js.includes('aria-label="Close"'), "JS: close aria-label");
    assert(js.includes('aria-label="Toggle sidebar"'), "JS: toggle aria-label");
    assert(
      !js.includes('tabindex="0"'),
      "JS: no tabindex=0 on interactive elements",
    );
    assert(
      !js.includes('role="button"'),
      "JS: no role=button on interactive elements",
    );
    ...
    assert(
      js.includes('<button class="nav-item"'),
      "JS: nav <button> elements",
    );
