      ...
      (res) => {
        ...
        res.on('end', () => {
          ...
          try {
            ...
            /* empty - invalid JSON is expected */
          }
...
async function run() {
  ...
  try {
    ...
    r = await req('POST', '/api/auth/login', { body: { username: 'nobody', password: 'x' } });
    ...
    // ═══ SESSION SECURITY ═══
    console.log('\n>>> Session Security');
    r = await req('POST', '/api/auth/login', { body: { username: 'admin', password: '0000' } });
    ...
    if (newId) {
      r = await req('PUT', `/api/qa/${newId}`, { cookie, body: { title: 'Updated' } });
